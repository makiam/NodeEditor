...
public class BoxComponentPane<T extends BoxComponent, U extends BoxComponentPane> extends JComponent {
    ...
    private BoxComponentModel<T> model;
    private List<T> mSelectedBoxes = new ArrayList<>();


    public BoxComponentPane(BoxComponentModel model) {
        this.model = model;
        setupListeners();
    }
    ...
    public List<T> getSelectedBoxes() {
    ...
    public U center() {
        ...
        Rectangle bounds = new Rectangle(model.getComponents().get(0).getBounds());
        for (BoxComponent box : model.getComponents()) {
            box.layout();
